
import requests
import urllib3
from urllib3.exceptions import InsecureRequestWarning
# 禁用SSL警告
urllib3.disable_warnings(InsecureRequestWarning)
import base64
import hashlib
import re
from bs4 import BeautifulSoup

map = {
    "data:image/png;base64,":3,
    "data:image/png;base64,":6,
    "data:image/png;base64,":1,
    "data:image/png;base64,":8,
    "data:image/png;base64,":7,
    "data:image/png;base64,":0,
    "data:image/png;base64,":2,
    "data:image/png;base64,":9,
    "data:image/png;base64,":4,
    "data:image/png;base64,":5
}


cookies = {
    'm': '85c3a103a042f2da0b003fac9b8cf29e|1758898514000',
    'sessionid': 'g2hqbpd5sx2l3z9ei7fww49in2komemy',
    'qpfccr': 'true',
    'no-alert3': 'true',
    'tk': '-5942336745486126056',
}

headers = {
    'accept': 'application/json, text/javascript, */*; q=0.01',
    'accept-language': 'zh-CN,zh;q=0.9',
    'priority': 'u=0, i',
    'referer': 'https://match.yuanrenxue.cn/match/4',
    'sec-ch-ua': '"Chromium";v="140", "Not=A?Brand";v="24", "Google Chrome";v="140"',
    'sec-ch-ua-mobile': '?0',
    'sec-ch-ua-platform': '"Windows"',
    'sec-fetch-dest': 'empty',
    'sec-fetch-mode': 'cors',
    'sec-fetch-site': 'same-origin',
    'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/140.0.0.0 Safari/537.36',
    'x-requested-with': 'XMLHttpRequest',
    # 'cookie': 'm=85c3a103a042f2da0b003fac9b8cf29e|1758898514000; sessionid=g2hqbpd5sx2l3z9ei7fww49in2komemy; qpfccr=true; no-alert3=true; tk=-5942336745486126056',
}



def get_page_sum(html_content, filter):
    page_sum = 0
    soup = BeautifulSoup(html_content, 'html.parser')

    # 找到所有的td标签
    td_tags = soup.find_all('td')

    for td in td_tags:
        # 找到当前td内的所有img标签
        img_tags = td.find_all('img')

        if len(img_tags) >= 1:
            # 提取每个img的left值并排序
            img_filter = []
            img_left = {}
            for img in img_tags:
                # 过滤出class中不包含filter的img标签
                img_class = img.get('class', '')
                if filter not in img_class:
                    img_filter.append(img)
            index = 0
            for img in img_filter:
                # 从style属性中提取left值
                style = img.get('style', '')
                src = img.get('src', '')
                left_match = re.search(r'left:([-\d.]+)px', style)
                left_value = float(left_match.group(1))
                # 计算图片的left值,并添加到字典中
                left = 11 * index +left_value
                index += 1
                img_left[left] = map[src]

            # 返回一个按left排序的列表
            sorted_left = sorted(img_left.items())
            # 对排序后的left值 对应的数字进行拼接
            value = ''.join([f'{value}' for key, value in sorted_left])

            page_sum += int(value)
    return page_sum


# print(get_page_sum(
#     '<td><img src=\"data:image/png;base64,\" class=\"img_number e6bf6b9cd8a543d25dcd1dddbc69668e\" style=\"left:0px\"><img src=\"data:image/png;base64,\" class=\"img_number e6bf6b9cd8a543d25dcd1dddbc69668e\" style=\"left:11.5px\"><img src=\"data:image/png;base64,\" class=\"img_number e6bf6b9cd8a543d25dcd1dddbc69668e\" style=\"left:-11.5px\"><img src=\"data:image/png;base64,\" class=\"img_number e6bf6b9cd8a543d25dcd1dddbc69668e\" style=\"left:0.0px\"></td>'
#     ,'fb52dee46c19c26a3c804bcc2f8c2159'
# ))
total_sum = 0
for i in range(1, 6):
    params = {
        'page': str(i),
    }

    response = requests.get('https://match.yuanrenxue.cn/api/match/4', params=params, cookies=cookies, headers=headers)
    # 提取数字,并添加到列表中
    info = response.json()['info']
    key = response.json()['key']
    value = response.json()['value']
    # 对key+value进行base64编码
    base64_data = base64.b64encode(str(key+value).encode())
    # 替换编码后的字符串中的等号
    replace_data = base64_data.replace(b'=', b'')
    # 对编码后的字符串进行MD5哈希
    result = hashlib.md5(replace_data).hexdigest()
    # 计算当前页的总和并累加到总总和中
    total_sum += get_page_sum(info,result)

print(total_sum)

