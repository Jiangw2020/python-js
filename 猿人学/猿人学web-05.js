...
function get_m() {
     var m_list = []
    for (let i = 0; i < 4; i++) {
        m_list.push(_0x474032(_0x12eaf3()))
    }
    is = _0x2d5f5b().toString()
    m = _0x474032(is);
    m_list.push(m)
    var _$Ww = CryptoJS.enc.Utf8.parse(m_list.toString());
    var key = CryptoJS.enc.Utf8.parse(Base64.encode(is).slice(0, 16));

    RM4hZBv0dDon443M= CryptoJS.AES.encrypt(_$Ww, key, {
